/*		Name : Abhay Singh Khanka
		...
		The first change we make is at the first for loop we calculate the distance and storing it in an array. We also add a pragma for loop and vectorize the array using _mm_store_ps function. 
		The other change made in the second nested for loop is that instead of calculating CloseDist twice, we calculate it just once thus saving time.
*/
